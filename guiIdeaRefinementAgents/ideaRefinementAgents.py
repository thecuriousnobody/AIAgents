...
import io
from ideaRefinementAgentsSpawner import generate_agent_details
...
class GUIApp(tk.Tk):
    ...
    def create_notebook(self):
        ...
        idea_input_frame = ttk.Frame(notebook)
        notebook.add(idea_input_frame, text="idea Rough Cut Input")
        self.create_idea_input_tab(idea_input_frame)

        refined_idea_frame = ttk.Frame(notebook)
        notebook.add(refined_idea_frame, text="Refined idea")
        self.create_refined_idea_tab(refined_idea_frame)
    ...
    def create_agents_tab(self, agents_frame):
        ...
        default_goal = "Refine the rough cut idea create a compelling, nuanced, and factually accurate script that captivates my podcast audience"
        ...
        default_context = "I need to create a compelling script for an upcoming Podcast"
        ...
        set_button = ttk.Button(agents_frame, text="Set Goal and Context",
                                command=lambda: self.set_goal_and_context(self.goal_text.get('1.0', 'end-1c'),
                                                                          self.context_text.get('1.0', 'end-1c')))
        ...
        create_infra_button = ttk.Button(button_frame, text="Create Agent Infrastructure",
                                         command=self.create_agent_infrastructure)
        ...
        create_refined_idea_button = ttk.Button(button_frame, text="Create Refined idea", command=self.run_agents)
        create_refined_idea_button.pack(side="left", padx=5, pady=5)
    ...
    def create_idea_input_tab(self, idea_input_frame):
        idea_input_text = tk.Text(idea_input_frame, wrap="word")
        idea_input_text.pack(side="top", fill="both", expand=True)

        def set_idea_rough_cut():
            self.idea_rough_cut = idea_input_text.get("1.0", "end-1c")

        set_idea_button = ttk.Button(idea_input_frame, text="Set idea Rough Cut", command=set_idea_rough_cut)
        set_idea_button.pack(side="bottom", fill="x", padx=5, pady=5)

    def create_refined_idea_tab(self, refined_idea_frame):
        self.refined_idea_text = tk.Text(refined_idea_frame, wrap="word")
        self.refined_idea_text.pack(side="top", fill="both", expand=True)

        # Add a scrollbar
        scrollbar = ttk.Scrollbar(refined_idea_frame, orient="vertical", command=self.refined_idea_text.yview)
        scrollbar.pack(side="right", fill="y")
        self.refined_idea_text.configure(yscrollcommand=scrollbar.set)

        # Add the Save idea button
        save_idea_button = ttk.Button(refined_idea_frame, text="Save idea", command=self.save_refined_idea)
        save_idea_button.pack(side="bottom", pady=10)

    def save_refined_idea(self):
        content = self.refined_idea_text.get("1.0", tk.END).strip()
        if not content:
            messagebox.showinfo("Empty idea", "There's no content to save.")
            return

        file_path = filedialog.asksaveasfilename(
            defaultextension=".txt",
            filetypes=[("Text files", "*.txt"), ("All files", "*.*")],
            title="Save Refined idea"
        )
        ...
        if file_path:
            try:
                with open(file_path, 'w', encoding='utf-8') as f:
                    f.write(content)
                messagebox.showinfo("Success", f"idea saved successfully to {file_path}")
            except Exception as e:
                messagebox.showerror("Error", f"An error occurred while saving: {str(e)}")

    def refine_idea(self):
        # Implement the idea refinement process using the agents
        # Update the refined_idea_text with the refined idea content
        pass
    ...
    def create_agent_infrastructure(self):
        ...
        if not goal or not context:
            messagebox.showwarning("Missing Goal or Context",
                                   "Please provide both the goal and context before creating the agent infrastructure.")
            return

        num_agents, agent_details_list, idea_analysis = generate_agent_details(self.idea_rough_cut, goal, context)
        ...
        messagebox.showinfo("idea Analysis", idea_analysis)
    ...
    def edit_agent_role(self, event):
        ...
        if selected_item:
            ...
            save_button = ttk.Button(button_frame, text="Save",
                                     command=lambda: self.save_agent_role(selected_item,
                                                                          role_text.get("1.0", tk.END).strip(),
                                                                          edit_dialog, agent_index))
            ...
            try:
                ...
                for agent_info in agents_data:
                    if agent_info['llm_type'] == "ChatAnthropic":
                        ...
                        messagebox.showwarning("Unknown LLM",
                                               f"Unknown LLM type: {agent_info['llm_type']}. Using default.")
                    ...
                    self.agents.append(agent)
                    self.agents_treeview.insert("", "end", text=agent.role,
                                                values=(agent.backstory, agent_info['llm_model']))
    ...
    def run_agents(self):
        ...
        for agent in self.agents:
            try:
                task = Task(
                    description=f"Refine the rough cut idea based on your role: {agent.role}",
                    agent=agent,
                    expected_output=f"Refined idea content from {agent.role}"
                )
                tasks.append(task)
            except Exception as e:
        ...
        self.refined_idea_text.delete("1.0", "end")
        self.refined_idea_text.insert("end", final_output)
