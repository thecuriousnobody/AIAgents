...
import io
from podcastHelperAgentsSpawner import generate_agent_details
...
class GUIApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Podcast Preparation Assistant")
    ...
    def create_notebook(self):
        ...
        guest_background_frame = ttk.Frame(notebook)
        notebook.add(guest_background_frame, text="Guest Background Information")
        self.create_guest_background_tab(guest_background_frame)

        podcast_prep_frame = ttk.Frame(notebook)
        notebook.add(podcast_prep_frame, text="Podcast Prep Material")
        self.create_podcast_prep_tab(podcast_prep_frame)
    ...
    def create_agents_tab(self, agents_frame):
        ...
        create_podcast_material_button = ttk.Button(button_frame, text="Create Podcast Material and Script", command=self.run_agents)
        create_podcast_material_button.pack(side="left", padx=5, pady=5)
    
    def create_guest_background_tab(self, guest_background_frame):
        guest_background_text = tk.Text(guest_background_frame, wrap="word")
        guest_background_text.pack(side="top", fill="both", expand=True)

        def set_guest_background():
            self.guest_background = guest_background_text.get("1.0", "end-1c")

        set_guest_background_button = ttk.Button(guest_background_frame, text="Set Guest Background", command=set_guest_background)
        set_guest_background_button.pack(side="bottom", fill="x", padx=5, pady=5)

    def create_podcast_prep_tab(self, podcast_prep_frame):
        self.podcast_prep_text = tk.Text(podcast_prep_frame, wrap="word")
        self.podcast_prep_text.pack(side="top", fill="both", expand=True)
